...
- `ui/` — `SpectrumWaterfall` (DPR-crisp spectrum + integrated tuner scale with kind-colored detection carets, peak-hold, scrolling waterfall, wheel-zoom centered on cursor, hover freq/dB readout, click/shift-drag to tune/pan) with `WaterfallControls` (colormap + dB levels popover + auto-level), `colormaps`, and the canvas palette `theme.ts`; `TopBar` (ruled navbar + transport + theme toggle); `deck/ReceiverDeck` (VFO odometer with click-to-type entry, demod `toggle-group`, custom `Fader` sliders, `Meter` as an ElevenLabs Matrix dot-matrix VU, IQ constellation + ElevenLabs `BarVisualizer` fed by a `MediaStreamDestination` tap on the engine's audio graph); `DetectionsPanel` / `SignalLibrary` / `ScenarioPanel` in the chanhdai lined idiom behind `RailTabs` + a shadcn bottom `Sheet` on mobile; `StartOverlay`, `StatusBar`, `SnrHistory` (per-signal uPlot strip). Chrome is Tailwind v4 + shadcn/ui (zinc, light + dark); the design system is documented in `DESIGN.md`. Global keyboard shortcuts live in `hooks/useHotkeys.ts`.
- `academy/` — the RF Academy second view (`store.view`), three tabs: **Explorer** (`LogAxis` canvas log-frequency explorer with LOD label collision-skipping, `Inspector` with scenario handoff, `Lessons` live demos); **Course** (`course/` — The Radio Bench curriculum ported by `scripts/convert-course.mjs` into code-split generated data: 25 chapters of rich prose with glossary hover-links, 206 quiz questions with explanations, bench labs, localStorage progress, and try-it chips that load matching scenarios); **Reference** (`course/ReferenceView.tsx` — searchable 284-term glossary + `widgets/` playground: 12 slider-driven canvas interactives in our design system). Content attribution in `NOTICE`.
